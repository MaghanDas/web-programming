...
function drawRandomCard(){
 ...
 drawCardBtn.addEventListener("click", () => {
    if(cardsThisRound >= 8 || Object.values(cardCountsByType).some(v => v >= 5))  {
    ...
    if(cardCountsByType[currentCard] >= 5){
        drawCardBtn.disabled = true;
    }
 });
...
function startNextRound(){
    if(currentRound < MAX_ROUNDS) currentRound++;
    cardsThisRound = 0;
    cardsThisRoundEl.textContent = cardsThisRound;
    currentCard =null;
    ...
    roundSegments = [];
    document.getElementById("roundeIndex").textContent = currentRound;
...
// assume stations have 'district' property
    ...
    //     cardsThisRound = 0;
    //     cardCountsByType = {A:0, B:0, C:0,J:0};
    //     roundSegments = [];
    //     currentCard = null;
    //     cardDisplay.textContent = '-';
    //     cardsThisRoundEl.textContent = cardsThisRound;
