...
// ---------- Danube River and Region Rendering ----------
function renderDanubeRiver() {
  const boardWrap = document.querySelector('#boardWrap');
  
  // Create Danube River
  const danube = document.createElement('div');
  danube.className = 'danube-river';
  boardWrap.appendChild(danube);
  
  // Create region labels
  const budaLabel = document.createElement('div');
  budaLabel.className = 'region-label buda-label';
  budaLabel.textContent = 'BUDA';
  boardWrap.appendChild(budaLabel);
  
  const pestLabel = document.createElement('div');
  pestLabel.className = 'region-label pest-label';
  pestLabel.textContent = 'PEST';
  boardWrap.appendChild(pestLabel);
}
...
function renderBoard(sts) {
  ...
  // Add Danube River background to SVG
  const riverRect = document.createElementNS(SVG_NS, 'rect');
  riverRect.setAttribute('x', '0');
  riverRect.setAttribute('y', '450'); // Middle of 1000px board
  riverRect.setAttribute('width', '1000');
  riverRect.setAttribute('height', '100');
  riverRect.setAttribute('fill', '#4A90E2');
  riverRect.setAttribute('opacity', '0.3');
  boardEl.appendChild(riverRect);
  ...
  sts.forEach((st, idx) => {
    ...
    if (st.side) {
      g.setAttribute('data-side', st.side);
      // Add bridge class for stations near the river that can connect both sides
      if ((st.side === 'Buda' && st.y >= 4) || (st.side === 'Pest' && st.y <= 5)) {
        g.classList.add('bridge');
      }
    }
    ...
    // Different fill colors based on region
    if (st.side === 'Buda') {
      circle.setAttribute('fill', '#2C5AA0'); // Blue for Buda
    } else if (st.side === 'Pest') {
      circle.setAttribute('fill', '#E41F18'); // Red for Pest
    } else {
      circle.setAttribute('fill', '#111'); // Default
    }
...
function drawLineSVG(start, end, color = '#005CA5') {
  ...
  // Thicker lines for Danube crossings
  const crossesRiver = crossesDanube({a: start, b: end});
  line.setAttribute('stroke-width', crossesRiver ? '10' : '8');
  
  // Special styling for bridge segments
  if (crossesRiver) {
...
(async function init() {
  try {
    ...
    renderDanubeRiver(); // Add Danube River rendering
    resetDeck();
    ...
    // wire rules and back buttons
    const rulesBtn = document.querySelector('#rulesBtn');
    const backFromRules = document.querySelector('#backFromRules');
    
    if (rulesBtn) {
      rulesBtn.addEventListener('click', () => showScreen('rules'));
    }
    if (backFromRules) {
      backFromRules.addEventListener('click', () => showScreen('menu'));
    }
    ...
    if (scoresBtn) {
      scoresBtn.addEventListener('click', () => {
        // For now, just show a message - you can implement high scores later
        showScreen('menu');
        showMessage('High scores feature coming soon!');
      });
    }
