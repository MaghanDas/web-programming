...
function update(dt) {
   if (gameState === 1 || gameState === 2) {
    plane.x += plane.vx * dt;
    // Parcel follows plane until dropped
    if (gameState === 1) {
      parcel.x = plane.x + plane.width / 2 - parcel.width / 2;
      parcel.y = plane.y + plane.height;
    }    
  }
  if (gameState === 2) {
    parcel.vy += parcel.ay * dt;   // v = v + a·t
    parcel.y += parcel.vy * dt;    // y = y + v·t
...
function render() {
  ...
  ctx.drawImage(house.img, house.x,house.y,house.width,house.height);
   ...
   if (gameState === 3) {
    ctx.font = "40px Arial";
    ctx.fillStyle = "green";
    ctx.fillText("Delivered", canvas.width / 2 - 100, canvas.height / 2);
  } 

    if (gameState === 4) {
    ctx.font = "40px Arial";
    ctx.fillStyle = "green";
    ctx.fillText("Missed", canvas.width / 2 - 100, canvas.height / 2);
  } 
}
